...
export const RoomPage = () => {
  ...
  if (!room) {
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-4 text-center">
        ...
        <Button onClick={() => navigate('/')} className="bg-purple-600 hover:bg-purple-500 text-xs rounded-xl">
  ...
  const handlePlayQueueItem = (item: QueueItem) => {
    ...
    changeRoomMedia(room.id, item.url, item.title, ytDetails.thumbnail_url);
      ...
      <main className="w-full flex-1 p-2 md:p-4 grid grid-cols-1 lg:grid-cols-12 gap-3 md:gap-4 max-w-7xl mx-auto">
        ...
        <div className="lg:col-span-7 xl:col-span-8 flex flex-col gap-2 md:gap-3 w-full">
          <div className="flex items-center justify-between px-1 w-full">
            <Button
              ...
              variant="ghost"
              size="sm"
              className="text-slate-400 hover:text-white gap-1 text-xs px-2 h-7 rounded-xl"
            >
            ...
            <div className="flex items-center gap-1.5">
              <Button
                ...
                size="sm"
                variant="outline"
                className="border-purple-800 text-purple-300 hover:bg-purple-950 text-[11px] h-7 gap-1 rounded-xl"
              >
              ...
              {isHost && (
                <Button
                  ...
                  size="sm"
                  variant="destructive"
                  className="bg-red-600/80 hover:bg-red-600 text-white text-[11px] h-7 gap-1 font-semibold rounded-xl"
          ...
          <div className="bg-slate-950 rounded-3xl overflow-hidden w-full shadow-2xl border border-purple-900/30">
            <MediaPlayer
              room={room}
              isHost={isHost}
          ...
          <div className="lg:hidden flex items-center justify-between p-2.5 rounded-2xl bg-slate-900/90 border border-purple-900/40">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[10px] text-slate-400 truncate mt-0.5">
                Ведущий: <span className="text-pink-400 font-semibold">{room.host_name}</span>
              </p>
            </div>
            <div className="flex items-center gap-1 text-[10px] bg-purple-950/80 border border-purple-800/40 text-cyan-300 px-2 py-1 rounded-full font-bold ml-2 shrink-0">
              <Users className="h-3 w-3 text-cyan-400" /> {roomMembers.length}
            </div>
          </div>
        ...
        <div className="lg:hidden flex border border-purple-900/40 bg-slate-950/90 backdrop-blur-md rounded-2xl p-1 gap-1 my-1 w-full shadow-inner">
          <button
            ...
            className={`flex-1 py-2 text-[11px] font-bold rounded-xl flex items-center justify-center gap-1 transition-all duration-200 ${
          ...
          >
            ...
            <span>Очередь</span>
            <span className="text-[9px] bg-slate-950/60 px-1.5 py-0.2 rounded-full font-mono">{roomQueue.length}</span>
          </button>
          ...
          >
            ...
            <span>Люди</span>
            <span className="text-[9px] bg-slate-950/60 px-1.5 py-0.2 rounded-full font-mono">{roomMembers.length}</span>
          </button>
        ...
        <div className="lg:hidden flex flex-col h-[460px] w-full">
          ...
          {activeMobileTab === 'info' && (
            <div className="p-4 rounded-3xl border border-purple-900/40 bg-slate-900/90 space-y-3 w-full shadow-xl">
              <div>
              ...
              <div className="pt-2 border-t border-purple-950 flex items-center justify-between text-xs text-purple-300">
                <span className="flex items-center gap-1.5 font-bold">
                  <Radio className="h-3.5 w-3.5 text-emerald-400 animate-pulse" /> В эфире
                </span>
        ...
        <DialogContent className="bg-slate-900 text-slate-100 border-purple-900/60 sm:max-w-md">
          ...
          <DialogFooter className="mt-4 flex items-center justify-end gap-2">
            ...
            <Button
              ...
              className="bg-red-600 hover:bg-red-500 text-white font-semibold text-xs rounded-xl"
            >
              Да, удалить
            </Button>
          </DialogFooter>
...
<dyad-write path="src/components/RoomChat.tsx" description="Улучшен вид сообщений и поле ввода для мобильных устройств">
  ...
  return (
    <>
      <div className="relative flex flex-col justify-between h-full w-full bg-slate-900/95 rounded-2xl md:rounded-3xl border border-purple-900/40 overflow-hidden shadow-xl">
        {/* Подшапка чата */}
        <div className="p-2.5 border-b border-purple-900/30 flex items-center justify-between bg-slate-950/60 shrink-0">
          <span className="text-[11px] text-slate-400 flex items-center gap-1 font-semibold">
            <Sparkles className="h-3 w-3 text-pink-400" /> Чат комнаты ({validMessages.length})
          </span>
        </div>
                  ...
                  <div
                    className={`max-w-[82%] rounded-2xl px-3 py-2 ${
                      isMe
                        ...
                        : 'bg-slate-950/80 border border-purple-950 text-slate-200 rounded-tl-none'
                    }`}
                  >
                    <div className="flex items-center justify-between gap-2 mb-0.5">
                      {!isMe && (
                        <span
                          onClick={() => handleAuthorClick(msg)}
                          className="font-semibold text-[10px] text-pink-400 truncate max-w-[100px] cursor-pointer hover:underline"
                        >
                          {msg.user_name}
                        </span>
                      )}
        ...
        >
          <Input
            ...
            className="bg-slate-900 border-purple-900/50 text-xs text-slate-100 placeholder:text-slate-500 flex-1 h-9 rounded-full px-4 focus:border-pink-500"
          />
          <Button
            type="submit"
            size="icon"
            className="h-9 w-9 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 to-pink-500 text-white shrink-0 rounded-full shadow-md"
          >
