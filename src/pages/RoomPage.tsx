...
export const RoomPage = () => {
  ...
  const handleAddQueueItem = (url: string) => {
    ...
    const newItem: QueueItem = {
      id: `q-${Date.now()}`,
      room_id: room.id,
      title: mediaInfo.title || `Видео (${ytDetails.videoId})`,
      url: url,
      thumbnail_url: mediaInfo.thumbnail || ytDetails.thumbnail,
  ...
  const handlePlayQueueItem = (item: QueueItem) => {
    ...
    const mediaInfo = parseMediaUrl(item.url);
    const ytDetails = extractYouTubeDetails(item.url);
    const thumbnail = item.thumbnail_url || mediaInfo.thumbnail || ytDetails.thumbnail;

    changeRoomMedia(room.id, item.url, item.title, thumbnail);
            ...
            <div className="flex items-center gap-2 overflow-x-auto scrollbar-none py-0.5 px-1">
              {RAVE_REACTIONS.map((emoji) => (
                <button
                  ...
                  className="h-8 w-8 sm:h-9 sm:w-9 rounded-full bg-slate-950/60 border border-purple-900/40 hover:bg-purple-800/60 hover:border-pink-500/60 hover:scale-125 active:scale-90 text-base sm:text-lg flex items-center justify-center transition-all duration-150 shrink-0 outline-none focus:outline-none focus:ring-0 focus-visible:outline-none focus-visible:ring-0 select-none [-webkit-tap-highlight-color:transparent]"
          ...
          <Tabs value={sidebarTab} onValueChange={(val: any) => setSidebarTab(val)} className="flex flex-col h-full w-full min-h-0">
            <TabsList className="grid grid-cols-3 bg-slate-950 p-1 border-b border-purple-900/40 rounded-none shrink-0 h-11">
              ...
              <TabsTrigger
                ...
                className="text-xs font-semibold data-[state=active]:bg-purple-800 data-[state=active]:text-white rounded-lg flex items-center gap-1"
              >
                <ListMusic className="h-3.5 w-3.5 text-purple-300" /> Очередь ({roomQueue.length})
              </TabsTrigger>
              <TabsTrigger
                value="members"
                className="text-xs font-semibold data-[state=active]:bg-purple-800 data-[state=active]:text-white rounded-lg flex items-center gap-1"
