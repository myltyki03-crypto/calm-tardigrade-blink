...
export const RoomChat: React.FC<RoomChatProps> = ({
  ...
  return (
    <div className="relative flex flex-col h-full w-full bg-slate-900/95 overflow-hidden">
      {/* Анимация реакций */}
      ...
      {/* Форма отправки */}
      <form onSubmit={handleSend} className="p-2 border-t border-purple-900/40 bg-slate-950/90 flex items-center gap-1.5 w-full shrink-0">
        <Input
          placeholder="Напишите сообщение..."
          value={inputText}
          onChange={(e) => setInputText(e.target.value)}
          className="bg-slate-900 border-purple-900/50 focus:border-pink-500 text-xs text-slate-100 placeholder:text-slate-500 flex-1 h-8 rounded-xl"
        />
        <Button
          type="submit"
          size="icon"
          className="h-8 w-8 bg-pink-600 hover:bg-pink-500 text-white shrink-0 rounded-xl"
        >
