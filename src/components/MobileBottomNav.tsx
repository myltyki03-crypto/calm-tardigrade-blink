...
import { Home, Plus, Users, MessageSquare } from 'lucide-react';
...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({
  ...
  const { currentUser, friendRequests, unreadDirectCount } = useRooms();
    ...
    <>
      <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-slate-950/95 backdrop-blur-lg border-t border-purple-900/40 px-3 py-2 flex items-center justify-around">
        ...
        >
          ...
          <span>ЛС</span>
          {unreadDirectCount > 0 && (
            <span className="absolute -top-1 -right-2 h-4 w-4 rounded-full bg-pink-600 text-white font-black text-[9px] flex items-center justify-center animate-pulse shadow-md">
              {unreadDirectCount}
            </span>
          )}
        </button>
